...
class YOLOXAuxHead(nn.Module):
    ...
    def pair_hand_objects(self, cls_preds, bbox_preds):
        ...
        hands_x = hands[:,0] + hands[:,2]/2
        hands_y = hands[:,1] + hands[:,3]/2

        objs_x = objs[:,0] + objs[:,2]/2
        objs_y = objs[:,1] + objs[:,2]/2

        hands_points = torch.stack((hands_x, hands_y), dim=1)
        objs_points = torch.stack((objs_x, objs_y), dim=1)
