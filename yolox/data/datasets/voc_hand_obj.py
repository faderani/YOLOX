...
class AnnotationTransform(object):
    ...
    def __call__(self, target):
        ...
        overlaps = np.zeros((num_objs, len(self.class_to_ind)), dtype=np.float32)
        ...
        # ret_dict = {'boxes': boxes.astype(np.float32),
        ...
        #         'unitdx': unitdx.astype(np.float32),
        #         'unitdy': unitdy.astype(np.float32),
...
class VOCHandObjDetection(Dataset):
    ...
    def load_anno_from_ids(self, index):
        ...
        arr, img_info = self.target_transform(target)
        ...
        return (arr, img_info, resized_info)
